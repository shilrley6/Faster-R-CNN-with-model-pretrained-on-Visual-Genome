...
class vg(imdb):
    ...
    def _do_python_eval(self, output_dir, pic=True, eval_attributes = False):
        # use pic to replace pickle
        ...
        with (open('results.txt', 'a')) as f:
            f.write('Mean AP = {:.4f} \r\n'.format(np.mean(aps)))
            f.write('Weighted Mean AP = {:.4f} \r\n'.format(np.average(aps, weights=weights)))
            f.write('Mean Detection Threshold = {:.3f} \r\n'.format(avg_thresh))
            f.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ \r\n')
